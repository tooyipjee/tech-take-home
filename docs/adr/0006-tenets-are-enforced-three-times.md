# 0006 — Tenets are enforced three times, and a violation halts one capability
...
**Tenets** are SQL statements over committed state that must return no rows. They live in
`packages/kernel/src/tenets.ts`, are platform-owned, and are **derived rather than curated**:
...
2. **The transaction.** Postconditions re-derive the tenets inside the writing transaction,
...
3. **The clock.** A reconciler runs every tenet on a timer and records each run in
   `tenet_runs`. Anything it finds arrived by a path the runtime never saw — a manual UPDATE,
   a restore, a migration, a kernel bug. That is precisely the class of failure an audit log
   cannot tell you about.

Expressing a tenet as SQL rather than as a TypeScript assertion is what makes the three layers
...
**On violation, the platform halts the capabilities the tenet names** — writes to them return
`halted`; reads and unrelated capabilities keep serving. Only an admin (`tenets:clear`) can
resume, and only after every tenet guarding that capability passes again.
...
- **Per-capability invariants declared by app authors.** Scales to capabilities we have not
  imagined, but a tenet that app work can define is a tenet app work can weaken. Tenets are
...
- A false-positive tenet takes a capability offline. The cost of a wrong invariant is now
...
- Thresholds in tenets are read from `capability_registry`, so an invariant cannot drift from
...
- `npm test` fails if a derived tenet is not named by a database test, and `npm run lint` fails
  if a change edits the platform without a record under `docs/platform-changes/`.
- The reconciler adds a query load proportional to tenet count every 15s. At this data volume
  it is irrelevant; at real volume tenets need incremental formulations or a windowed scan.
