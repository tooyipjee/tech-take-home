# Platform change: deterministic tenets, postconditions and reconciliation

Tier: 2 (extends the framework)

## What changed

- `packages/db/migrations/0003_tenets.sql` — every effect row carries the id of the
  ...
  (`tenet_runs`, `capability_halts`) exists.
- `packages/kernel/src/tenets.ts` — the tenet derivation: SQL statements that must
...
| Tenet | Derived from | Enforced by |
| --- | --- | --- |
...
`packages/kernel/test/tenets.test.ts` asserts the derived set for the current
declarations, that thresholds are not hard-coded, that a money-moving capability
without an effect is refused at registration, and that no tenet is in force
without a test naming it.

`packages/kernel/test/db/invariants.test.ts` attacks each tenet by a different
...
idempotency tenets, which the runtime alone would never allow.

## Rollback

The migration is additive. Reverting the kernel changes disables the postcondition
and the reconciler; the database-level tenets stay in force, which is the intended
failure mode. To retire a database tenet, a further tier-2 migration must drop the
...
- A halt is scoped to the capabilities the violated tenet names. Reads keep serving.
- Only `tenets:clear` (admin) can resume a capability, and only after every tenet
