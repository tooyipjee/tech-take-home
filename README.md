...
the header is a mock identity provider — it picks the principal (Avery, agent · Sam, supervisor ·
Robin, admin), the API reads it from the `x-platform-user` header, and it stands in for an
OAuth2/OIDC sign-in (swapping it for the real thing changes `resolvePrincipal` and nothing else).
Tile availability is presentation only; the runtime re-checks scopes on every capability call.
